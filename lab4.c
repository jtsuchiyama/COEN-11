...
typedef union extra // Holds more information for a party, however, a reference can only access one member
{
...
typedef struct entry // Declares members of the structure, which collect the data of the parties 
{
	char name[LENGTH];
	int size;
	int occasion;
...
ENTRY nlist[SIZE]; // Holds the references of the structure
int counter=0; // Keeps track of how many parties are on the waiting list
...
int main()
{
	while (1) 
	{
		...
		scanf("%d", &input);
		switch(input) // Main branch for the functions of the code
		{
			case 1:
				insert();
				break;
	...
	else
	{
		...
		if (flag==1) // Only satisfied if there were no duplicates found 
		{
			...
			{	
				...
				printf("What is the occasion? \n Inputs \n 1 - Celebrating a birthday \n 2 - Celebrating an anniversary \n Anything other number - No occasion \n");
				scanf("%d", &response); // Grabs the occasion input
				p->occasion=response; // Stores the occasion input to be accessed for the list function
				switch(response) // Branch for the different occasions
				{
					case 1:
						printf("What is the name of the birthday person? \n");
						scanf("%s",p->extra_info.birthday_name);
						break;
					case 2:
						printf("What year anniversary are you celebrating? \n");
						scanf("%d",&p->extra_info.years);
						break;
					default:
						printf("What is the average age of the party \n");
						scanf("%f",&p->extra_info.average_age);
						break;
				}
		...
		if(amount==0) // Checks that the opening is bigger than zero
			...
			for(i=0;i<counter;i++,p++) 
			{
				...
				if(amount>=p->size) // Finds a suitable group
				{
					...
					amount-=p->size; 
					for(j=i+1;j<counter;j++) // Shifts all the data depending on the occasion
					{
						p = &nlist[j];
						q = &nlist[j-1];
						strcpy(q->name, p->name);
						q->size=p->size;
						q->occasion=p->occasion;
						if(p->occasion==1)
							strcpy(q->extra_info.birthday_name,p->extra_info.birthday_name);
						else if (p->occasion==2)
							q->extra_info.years=p->extra_info.years;
						else 
							q->extra_info.average_age=p->extra_info.average_age;
					}
					counter--; // Updates the amount of parties
...
void list()
{
	...
	{
		printf("LIST: \n");
		for(i=0; i<counter; i++,p++) // Lists the parties with the text changing depending on what the occasion is
		{
			if(p->occasion==1)
				printf("Party #%d: %s for %d people celebrating %s's birthday (Occasion %d) \n", i+1, p->name, p->size, p->extra_info.birthday_name, p->occasion);
			else if(p->occasion==2)
				printf("Party #%d: %s for %d people celebrating an anniversary of %d years (Occasion %d) \n", i+1, p->name, p->size, p->extra_info.years, p->occasion);
			else
				printf("Party #%d: %s for %d people with an average age of %f years old (Ocassion %d) \n", i+1, p->name, p->size, p->extra_info.average_age, p->occasion);
		}
	}	
}	
